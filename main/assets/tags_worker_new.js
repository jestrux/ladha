...
self.onmessage = function(e){
    ...
    id3({ file: song_path, type: id3.OPEN_LOCAL }, function(err, tags) {
        if(tags){
            ...
            var pic = tags.v2.image;
            if(pic && pic.data){
                var mime = pic.mime.split("/")[1];
                url = base64ArrayBuffer(pic.data, mime);

                song.artwork = {url: url, mime: mime};
            }else{
                song.artwork = "no atwork";
            }
        }    
...
function base64ArrayBuffer(arrayBuffer, mime) {
    var base64    = ''
